    ...
    for _ in range(count):
        ...
        for i,layer in enumerate(model.layers):
            if isinstance(layer, keras.layers.Conv2D) and i != 0:
                chromosome.extend([np.random.choice([0,1], p=[0.3,0.7]) for _ in range(layer.filters)])
            elif isinstance(layer, keras.layers.Dense) and i != len(model.layers)-1:
                chromosome.extend([np.random.choice([0,1], p=[0.3,0.7]) for _ in range(layer.units)])
